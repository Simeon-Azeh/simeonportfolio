...
const ChatWidget = () => {
  ...
  return (
    <>
      ...
      <motion.button
        onClick={toggleChat}
        className="fixed bottom-6 right-6 md:right-20 z-50 bg-gradient-to-r from-violet-500 to-purple-600 dark:from-violet-500 dark:to-purple-600 text-white w-14 h-14 rounded-full flex items-center justify-center shadow-lg shadow-violet-500/25 hover:shadow-xl hover:shadow-violet-500/30 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 dark:focus:ring-offset-zinc-900"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 z-50 w-[350px] md:w-[400px] h-[520px] bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl shadow-violet-500/10 flex flex-col overflow-hidden border border-slate-200 dark:border-zinc-800"
          >
            {/* Chat widget header */}
            <div className="bg-gradient-to-r from-violet-500 to-purple-600 text-white p-4 flex justify-between items-center relative overflow-hidden">
            ...
            <button
              ...
              className="md:hidden flex items-center justify-center space-x-1 py-2 bg-slate-50 dark:bg-zinc-950 text-sm text-slate-600 dark:text-slate-300 border-b border-slate-200 dark:border-zinc-800"
            >
            ...
            <div className="flex flex-1 overflow-hidden">
              ...
              <AnimatePresence>
                {(showHistory || window.innerWidth >= 768) && (
                  <motion.div
                    ...
                    transition={{ duration: 0.2 }}
                    className="w-1/3 border-r border-slate-200 dark:border-zinc-800 overflow-y-auto bg-slate-50 dark:bg-zinc-900"
                  >
                    <div className="p-3 border-b border-slate-200 dark:border-zinc-800">
                      <h4 className="text-xs uppercase font-semibold text-slate-500 dark:text-slate-400">{t('chat_history', 'Chat History')}</h4>
                    </div>
                    <div className="divide-y divide-slate-200 dark:divide-zinc-800">
                      {chatHistory.map(chat => (
                        <div key={chat.id} className="flex items-center justify-between p-3 hover:bg-slate-100 dark:hover:bg-zinc-800 cursor-pointer transition-colors">
                          <div
                            className="flex-1 truncate"
                            onClick={() => loadChat(chat.id)}
                          >
                            <div className="flex items-center">
                              <p className={`text-sm font-medium truncate ${currentChat?.id === chat.id ? 'text-violet-600 dark:text-violet-400' : 'text-slate-700 dark:text-slate-200'}`}>
                          ...
                          <button
                            ...
                            className="p-1.5 text-slate-500 hover:text-red-500 dark:text-slate-400 dark:hover:text-red-400 bg-slate-100 dark:bg-zinc-800 rounded-full"
              ...
              <div className="flex-1 flex flex-col overflow-hidden">
                {/* Messages container */}
                <div className="flex-1 p-4 overflow-y-auto bg-slate-50/50 dark:bg-zinc-950">
                    ...
                    {isTyping && (
                      <div className="flex justify-start">
                        <motion.div
                          initial={{ opacity: 0 }}
                          animate={{ opacity: 1 }}
                          className="bg-white dark:bg-zinc-800 rounded-2xl px-4 py-3 text-slate-800 dark:text-white rounded-bl-none max-w-[75%] shadow-sm"
                        >
                ...
                <div className="p-3 border-t border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900">
                  <form onSubmit={handleSubmit} className="flex items-center">
                    <input
                      ...
                      className="flex-1 py-2.5 px-4 bg-slate-100 dark:bg-zinc-800 border-none rounded-l-full text-slate-800 dark:text-white placeholder-slate-500 dark:placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-opacity-50"
                    />
                    <button
                      type="submit"
